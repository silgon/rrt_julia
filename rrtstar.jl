...
function transition(c_v, p2::Array{Float64,1})
    p1 = nodes[c_v]
    if c_v == id_start
        if dist(p1, p2)< ε
            return p2
        ...
        p_v = in_edges(c_v, g)[1].source  # only previous vertex
        pp = nodes[p_v]
        d = rand()*(ε-.1)+.1
        theta = atan2(p1[2]-pp[2], p1[1]-pp[1])
        α = theta + randn()*.3
        return Float64[p1[1] + d*cos(α), p1[2] + d*sin(α)]
    end
end
